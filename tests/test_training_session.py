...
class TrainingSessionTests(unittest.TestCase):
    ...
    def test_timing_diagnostics_write_per_instance_and_multi_instance_csvs(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            from src.training.model_registry import TrainingModelRepository
            ...
            session = _NoSaveSession(
                ...
                instance_id=7,
                running_instances_provider=lambda: 3,
                multi_instance_events_path=root / "user" / "events.csv",
            )
            session.run_synchronously(max_batches=1)

            with (root / "user" / "Earthling-01.instance-07.timings.csv").open(
                newline="",
                encoding="utf-8",
            ) as file:
                timing_rows = list(csv.DictReader(file))
            with (root / "user" / "events.csv").open(
                newline="",
                encoding="utf-8",
            ) as file:
        ...
        row = timing_rows[0]
        self.assertEqual(row["Run ID"], session.run_id)
        ...
        self.assertEqual(row["Running Instances Start"], "3")
        self.assertEqual(row["Running Instances End"], "3")
        self.assertEqual(row["Average Loss"], "0.125000")
        self.assertIn("observation_seconds", row)
        self.assertIn("optimization_seconds", row)
        self.assertGreaterEqual(float(row["Total Seconds"]), 0.0)

        event_names = [event["Event"] for event in event_rows]
        self.assertIn("batch_start", event_names)
        self.assertIn("batch_end", event_names)
        self.assertIn("save_start", event_names)
        self.assertTrue(all(event["Run ID"] == session.run_id for event in event_rows))

    def test_batch_runner_does_not_need_to_accept_timing_keyword(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            from src.training.model_registry import TrainingModelRepository
