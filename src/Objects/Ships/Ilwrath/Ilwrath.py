...
class Ilwrath(SpaceShip):
    ...
    def __init__(self, ship_name, player_num):
        ...
        if Ilwrath.shared_sprites is None:
            Ilwrath.shared_sprites = []
            for i in range(Const.SHIP_DIRECTIONS):
                sprite_path = self.sprite_location.joinpath(f'{ship_name}{i:02d}.png')
                Ilwrath.shared_sprites.append(pygame.image.load(str(sprite_path)).convert_alpha())
    ...
    def perform_action1(self):
        if self.can_action1():
            ...
            projectile = Projectile("IlwrathA1", self)
