...
class PkunkA1(Projectile):
    ...
    def __init__(self, parent):
        ...
        if PkunkA1.shared_sprites is None:
            # Load sprites
            PkunkA1.shared_sprites = []
            for i in range(Const.SHIP_DIRECTIONS):
                sprite_path = self.sprite_location.joinpath(f'{self.name}{i:02d}.png')
                PkunkA1.shared_sprites.append(pygame.image.load(str(sprite_path)).convert_alpha())
            ...
            if self.death_anim > 0:
                ...
                for i in range(self.death_anim):
                    try:
                        ...
                        PkunkA1.death_anim.append(pygame.image.load(str(death_path)).convert_alpha())
                    except pygame.error:
                        break
            ...
            try:
                ...
                PkunkA1.launch_sound = pygame.mixer.Sound(str(sound_path))
            except pygame.error:
                PkunkA1.launch_sound = None