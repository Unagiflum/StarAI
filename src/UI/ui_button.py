...
class ToggleButton(Button):
    def __init__(
        ...
        active_color=ui.BRIGHT_GREEN,
        text_color=ui.WHITE,
        hover_color=ui.MENU_BUTTON_COLOR_HI,
    ):
        super().__init__(
            x, y, width, height, text, callback, bg_color, hover_color, text_color
        )
        self.is_on = initial_state
        self.active_color = (
            (*active_color, 255) if len(active_color) == 3 else active_color
        )
    ...
    def draw(self, surface, font):
        ...
        switch_color = (
            self.active_color
            if self.is_on
            else (*ui.DARK_RED, 255) if len(ui.DARK_RED) == 3 else ui.DARK_RED
        )
        switch_surface = pygame.Surface(
            (self.switch_rect.width, self.switch_rect.height), pygame.SRCALPHA
        )
        pygame.draw.rect(
            switch_surface,
            switch_color,
            switch_surface.get_rect(),
            border_radius=self.switch_height // 2,
        )
        surface.blit(switch_surface, self.switch_rect)

        button_x = (
            self.switch_rect.right - self.button_width - 2
            if self.is_on
            else self.switch_rect.x + 2
        )
        button_rect = pygame.Rect(0, 0, self.button_width - 2, self.button_height - 4)
        button_surface = pygame.Surface(
        ...
        pygame.draw.rect(
            ...
            button_color,
            button_rect,
            border_radius=self.button_height // 2,
        )
...
class KeyBinding(Button):
    ...
    def draw(self, surface, font):
        ...
        label_rect = label_surf.get_rect(
            midright=(
                self.rect.x - int(0.01 * Const.SCREEN_WIDTH),
