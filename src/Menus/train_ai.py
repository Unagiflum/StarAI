...
TRAINING_DEVICE_TOOLTIP = (
    "PyTorch device: choose which processor runs this training instance. "
    "Auto lets the training mode and available hardware choose."
)
GO_TO_BATCH_TOOLTIP = (
    "Go to batch: stop automatically after completing this absolute batch "
    "number. Enter 0 to run indefinitely."
)
GO_TO_EPSILON_TOOLTIP = (
    "Go to Epsilon: stop automatically when the current epsilon decays to "
...
OPPONENT_CONTROL_TOOLTIPS = (
    (
        ...
        "by a trained AI. The remainder uses the simple behavior below."
    ),
    (
        "Forward Activity: set how actively the simple opponent toggles its "
        "thrust control."
    ),
    (
        "A1 Activity: set how actively the simple opponent uses its primary "
        "ability."
    ),
    (
        "A2 Activity: set how actively the simple opponent uses its secondary "
        "ability."
    ),
    (
        "Face opponent: set how consistently the simple opponent turns toward "
        "the trainee."
    ),
)
REWARD_CONTROL_TOOLTIPS = {
    "Kill enemy": "Reward or penalize defeating the enemy ship.",
    "Reduce enemy crew": "Reward or penalize each point of crew damage dealt.",
    "Debuff enemy": "Reward or penalize applying a debuff to the enemy.",
    "Kill enemy object": (
        "Reward or penalize destroying an object created by the enemy."
    ),
    "Gain crew": "Reward or penalize each point of crew the trainee gains.",
    "Gain battery": (
        "Reward or penalize each point of battery energy the trainee gains."
    ),
    "Be at high speed": (
        "Reward or penalize speed above the ship's normal maximum thrust speed."
    ),
    "Point A1 at enemy": (
        "Reward or penalize keeping the primary ability aimed at the enemy."
    ),
    "In A1 range": (
        "Reward or penalize keeping the enemy within primary-ability range."
    ),
    "Use A1": "Reward or penalize using the primary ability.",
    "Point A2 at enemy": (
        "Reward or penalize keeping the secondary ability aimed at the enemy."
    ),
    "In A2 range": (
        "Reward or penalize keeping the enemy within secondary-ability range."
    ),
    "Use A2": "Reward or penalize using the secondary ability.",
    "Destroy own object": (
        "Reward or penalize the destruction of an object created by the trainee."
    ),
    "Lose battery": (
        "Reward or penalize each point of battery energy the trainee loses."
    ),
    "Battery at zero": (
        "Reward or penalize time spent with no battery energy."
    ),
    "Get debuffed": "Reward or penalize the trainee receiving a debuff.",
    "Lose crew": "Reward or penalize each point of crew damage received.",
    "Die": "Reward or penalize the trainee ship being defeated.",
}
REGIMEN_CONTROL_TOOLTIPS = (
    (
        "Match frame length: set the maximum simulated frames allotted to each "
        "opponent matchup."
    ),
    (
        "Rounds per batch: set how many complete opponent rotations are "
        "collected before each optimization batch."
    ),
    (
        "Batch grouping: set how many completed batches are grouped for model "
        "saves, rolling metrics, and CSV output."
    ),
    (
        "Replay size: set the maximum number of training samples retained for "
        "later optimization. The label shows the maximum samples generated by "
        "one batch; the value also shows estimated memory use."
    ),
    (
        "Starting Epsilon: set the initial chance of exploratory actions. The "
        "value in parentheses is the current epsilon."
    ),
    (
        "Epsilon floor: set the lowest exploration chance allowed as epsilon "
        "decays."
    ),
    (
        "Epsilon decay: multiply the current epsilon by this value after each "
        "completed batch."
    ),
    (
        "Epsilon frame span: set how many frames share each exploration "
        "decision and exploratory action."
    ),
    (
        "Gamma: set how strongly future rewards contribute to the training "
        "return."
    ),
    (
        "Minibatch size: set how many replay samples are used in each gradient "
        "update."
    ),
    (
        "Gradient steps: set how many model updates run after each batch. The "
        "value also shows the update-to-data ratio (UTD)."
    ),
    (
        "Learning rate: set the optimizer step size used for model updates."
    ),
    (
        "Hidden layer size: set the number of neurons in each hidden network "
        "layer."
    ),
    (
        "Hidden layer count: set the number of hidden layers in the value "
        "network."
    ),
)
...
def validate_coordinated_batch_start(
    ...
    *,
    ...
    if torch_obj is None or not cuda_ok:
        return CoordinatedBatchValidation(
            blocking_reason="GPU PyTorch is required for synced training",
            blocking_code="cuda",
        )
    ...
    if not device_keys or device_keys[0] != torch_backend.DEVICE_GPU:
        return CoordinatedBatchValidation(
            blocking_reason="Synced training requires a CUDA/GPU device",
            blocking_code="device",
        )
...
def _reward_control_tooltip(reward_key, display_label=None):
    """Return reward help while respecting ship-specific presentation labels."""

    if display_label == "Maintain A2":
        return "Reward or penalize keeping the secondary ability active."
    return REWARD_CONTROL_TOOLTIPS.get(
        reward_key,
        f"Reward or penalize {str(display_label or reward_key).lower()}.",
    )
...
def run(screen: pygame.Surface, menu_sound_manager=None, audio_service=None):
    ...
    slot_fields = [
        TextField(
            (
                row.x + 64,
                row.y + 3,
                row.width - 64 - SLOT_DELETE_BUTTON_WIDTH - 4,
                row.height - 6,
            )
        )
    ...
    delete_buttons = [
        ui_button.Button(
            row.right - SLOT_DELETE_BUTTON_WIDTH,
            row.y + 3,
            SLOT_DELETE_BUTTON_WIDTH,
            row.height - 6,
            "X",
            lambda: None,
    ...
    trainee_content_height = max(ship_tile.bottom, instance_start_button_rect.bottom) + 12
    trainee_scroll_y = 0
    ...
    def load_selected_model_conditions():
        if instance_manager.any_instance_running():
            show_notice("Model conditions cannot be loaded while training is running")
            return
        multi_instance_load = (
            instance_manager.batch_scheduling.apply_to_all_open_instances
        )
    ...
    def eligible_independent_start_specs():
        ...
        for instance in instance_manager.instances:
            if instance_manager.is_running_or_stopping(instance):
                ...
                continue
            instance_state = instance.state
            if instance_state.selected_ship is None or instance_state.selected_slot is None:
                skipped_count += 1
                continue
            model_slot = model_repository.slot_for(
                instance_state.selected_ship,
                instance_state.selected_slot,
            )
            ...
            changed = bool(
                model_slot.is_user
                and (
                    changed_training_groups(
                        existing_metadata.get("training", {}),
                        new_training,
                    )
                    or existing_metadata.get("description", model_slot.description)
                    != description
                )
            )
            specs.append((instance, model_slot, reset_checkpoint, changed))
        return tuple(specs), skipped_count

    def launch_independent_start_specs(specs, skipped_count=0):
        if instance_manager.coordinated_run_active() or instance_manager.any_instance_running():
            show_notice("Training is already in progress")
            return
        instance_slots = [
            (instance, model_slot)
        ...
        def launch_one(index):
            ...
            try:
                architecture = architecture_metadata_for(instance_state)
                updated_training = training_metadata_for(
                    instance_state,
                    reset_checkpoint=reset_checkpoint,
                )
                existing_metadata = (
                ...
                metadata = metadata_from_state(
                    ...
                    description=instance_state.slot_labels[
                        instance_state.selected_slot - 1
                    ].strip(),
                    architecture=architecture,
                    training=updated_training,
                    progress=_progress_for_model_update(
    ...
    def start_all_eligible_models():
        sync_state_from_ui()
        specs, skipped_count = eligible_independent_start_specs()
        if not specs:
            show_notice("No eligible training instances are ready to start")
            return
        reset_count = sum(1 for _instance, _slot, reset, _changed in specs if reset)
        changed_count = sum(
            1 for _instance, _slot, _reset, changed in specs if changed
        )

        def launch():
            launch_independent_start_specs(specs, skipped_count)

        if reset_count:
            confirmation_prompt[0] = ConfirmationPrompt(
                (
                    f"Start all {len(specs)} eligible instances? This will reset "
                    f"{reset_count} incompatible saved model"
            ...
            confirmation_prompt[0] = ConfirmationPrompt(
                (
                    f"Start all {len(specs)} eligible instances and update saved "
    ...
    def start_synced_models():
        if instance_manager.coordinated_run_active():
            ...
            confirmation_prompt[0] = ConfirmationPrompt(
                "Do you want to stop the synced training run?",
                stop_synced_training,
            )
            return
        if instance_manager.any_instance_running():
            confirm_stop_all_independent_training()
    ...
    start_stop_button = ui_button.Button(
        instance_start_button_rect.x,
        instance_start_button_rect.y,
    ...
    def control_tooltip_at(mouse_pos, batch_validation):
        ...
        top_level_targets = (
            ...
            (
                ...
                "Help: show concise guidance when hovering over training controls.",
            ),
            (
        ...
        active_tab = instance_manager.active_tab
        content_targets = []
        scroll_y = 0
        if active_tab == "trainee":
            ...
            content_targets.extend(
                (
                    (
                        ...
                        (
                            "Sync CPU runs: keep independently started CPU "
                            "instances at a shared batch pace and use their "
                            "average epsilon."
                        ),
                    ),
            ...
            content_targets.extend(
                (
                    field.rect,
                    (
                        "AI slot name: give the selected slot a name before "
                        "training can start."
                    ),
                )
                for field in slot_fields
            ...
            scroll_y = trainee_scroll_y
        elif active_tab == "opponent":
            content_targets.extend(
                (control.rect, label)
    ...
    while not exited[0]:
        ...
        for event in pygame.event.get():
            ...
            if instance_manager.active_tab == "trainee":
                ...
                start_stop_button.handle_event(translated, menu_sound_manager)
        ...
        independent_running_instances = (
            () if coordinated_active else tuple(instance_manager.running_instances())
        )
        independent_stop_capable = any(
            bool(getattr(instance_manager.status_for(instance), "running", False))
            and not bool(getattr(instance_manager.status_for(instance), "stopping", False))
        ...
        for slot_number, (delete_button, model_slot) in enumerate(
            ...
            delete_button.text = "X (ALL)" if apply_to_all else "X"
            if any_running or coordinated_active:
                delete_button.enabled = False
            elif apply_to_all:
                targets, _affected = eligible_user_model_deletion_targets(
                    instance_manager,
                    model_repository,
                    slot_number,
                )
                delete_button.enabled = bool(targets)
            else:
        ...
        multi_instance_load = apply_to_all
        if (
            is_currently_loaded
            ...
            if multi_instance_load:
                ...
                load_button.enabled = not any_running and any(
                    ...
                    and bool(getattr(candidate, "metadata", None))
                    for instance in instance_manager.instances
                )
            else:
        ...
        if coordinated_active:
            start_stop_button.text = (
                "Stopping synced" if coordinated_stopping else "Running synced"
            )
            ...
            eligible_specs, _skipped_count = eligible_independent_start_specs()
            start_stop_button.text = "Start all eligible"
            start_stop_button.enabled = bool(eligible_specs)
            ...
            start_stop_button.enabled = (
                ...
                and bool(state.slot_labels[state.selected_slot - 1].strip())
            )
            start_stop_button.bg_color = ui.OK_GREEN
            start_stop_button.hover_color = ui.OK_GREEN_HI
        ...
        if instance_manager.active_tab == "trainee":
            ...
            start_stop_button.draw(content, body_font, content_mouse_pos)
